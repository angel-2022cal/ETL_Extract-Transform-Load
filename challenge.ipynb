{
 "cells": [
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "Based on the output from using the regular line plot (which can plot date data), and change the style to only put dots by adding style='.' to the plot() method: movies_df[['release_date_wiki','release_date_kaggle']].plot(x='release_date_wiki', y='release_date_kaggle', style='.') Then, investigating the wild outlier around 2006using: movies_df[(movies_df['release_date_wiki'] > '1996-01-01') & (movies_df['release_date_kaggle'] < '1965-01-01')] it looks like somehow The Holiday in the Wikipedia data got merged with From Here to Eternity. We have to drop that row from our DataFrame. "
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "Both title options look pretty good, but the Kaggle data looks just a little bit more consistent. We confirmed there weren’t any missing titles in the Kaggle data with the following code:\n",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "The Wikipedia data is missing release dates for 11 movies, as seen, using: movies_df[movies_df['release_date_wiki'].isnull()] But the Kaggle data isn’t missing any release dates. In this case, we just dropped the Wikipedia data."
   ]
  },
  {
   "cell_type": "markdown",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "For the language data, we compared the value counts of each. We converted the lists in Language to tuples so that the value_counts() method will work. Using the code: movies_df['Language'].apply(lambda x: tuple(x) if type(x) == list else x).value_counts(dropna=False) For the Kaggle data, there was no lists, so we just ran value_counts() on it: movies_df['original_language'].value_counts(dropna=False) There’s a trade-off here between the Wikipedia language data and the Kaggle language data. While the Wikipedia data has more information about multiple languages, the Kaggle data is already in a consistent and usable format. Parsing the Wikipedia data may create too many difficulties to make it worthwhile, though. We chose to use the Kaggle data. The decision was based on saving time."
   ]
  },
  {
   "cell_type": "markdown",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "We looked at a small number of samples: movies_df[['Production company(s)','production_companies']]\n",
    "The Kaggle data is much more consistent, and it would be difficult, if not impossible, to translate the Wikipedia data into the same format. We decided to drop the Wikipedia data in this case."
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "Most of the runtimes are pretty close to each other, but the Wikipedia data has some outliers, so the Kaggle data is probably a better choice here. However, we can also see from the scatter plot: movies_df.fillna(0).plot(x='running_time', y='runtime', kind='scatter') that there are movies where Kaggle has 0 for the runtime but Wikipedia has data, so we filled in the gaps with Wikipedia data."
   ]
  },
  {
   "cell_type": "markdown",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "Since budget_wiki and budget_kaggle are numeric, we made a scatter plot to compare the values:\n",
    "movies_df.fillna(0).plot(x='budget_wiki',y='budget_kaggle', kind='scatter'). The Wikipedia data appears to have more outliers compared to the Kaggle data. However, there are quite a few movies with no data in the Kaggle column, while Wikipedia does have budget data. Therefore, we filled in the gaps with Wikipedia’s data."
   ]
  },
  {
   "cell_type": "markdown",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "Because we might be getting thrown off by the scale of the large data point using: movies_df.fillna(0).plot(x='box_office', y='revenue', kind='scatter'). We used a scatter plot for everything less than $1 billion in box_office.\n",
    "movies_df.fillna(0)[movies_df['box_office'] < 10**9].plot(x='box_office', y='revenue', kind='scatter') It looks similar to what we’ve seen for budget, so we made the same decision: keep the Kaggle data, but fill in the zeros with Wikipedia data."
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "Since we’ve merged our data and filled in values, we checked that there aren’t any columns with only one value, since that doesn’t really provide any information. We converted lists to tuples for value_counts() to work.\n",
    ...
    "    if num_values == 1:\n",
    "        print(col)\n",
    "Running this, we saw that 'video' only had one value:\n",
    "movies_df['video'].value_counts(dropna=False)\n",
    "Here’s what the output looked like.\n",
    "False    6044\n",
    "Name: video, dtype: int64\n",
    "Since it was false for every row, we didn’t include this column."
   ]
  },
  {
   "cell_type": "code",
   ...
   "outputs": [
    {
     ...
     "output_type": "stream",
     "text": [
      "C:\\Users\\User\\Anaconda3\\envs\\PythonData\\lib\\site-packages\\pandas\\core\\frame.py:4223: SettingWithCopyWarning: \n",
   ...
   "outputs": [
    {
     ...
     "output_type": "stream",
     "text": [
      "importing rows 0 to 1000000...Done. 216.62073945999146 total seconds elapsed\n",
      "importing rows 1000000 to 2000000...Done. 412.87334060668945 total seconds elapsed\n",
      "importing rows 2000000 to 3000000...Done. 610.5289463996887 total seconds elapsed\n",
      "importing rows 3000000 to 4000000...Done. 805.5002024173737 total seconds elapsed\n",
      "importing rows 4000000 to 5000000...Done. 999.6067686080933 total seconds elapsed\n",
      "importing rows 5000000 to 6000000...Done. 1193.52845287323 total seconds elapsed\n",
      "importing rows 6000000 to 7000000...Done. 1394.0158269405365 total seconds elapsed\n",
      "importing rows 7000000 to 8000000...Done. 1588.7840554714203 total seconds elapsed\n",
      "importing rows 8000000 to 9000000...Done. 1784.6872975826263 total seconds elapsed\n",
      "importing rows 9000000 to 10000000...Done. 1979.9106605052948 total seconds elapsed\n",
      "importing rows 10000000 to 11000000...Done. 2177.772360563278 total seconds elapsed\n",
      "importing rows 11000000 to 12000000...Done. 2373.3564767837524 total seconds elapsed\n",
      "importing rows 12000000 to 13000000...Done. 2568.7778465747833 total seconds elapsed\n",
      "importing rows 13000000 to 14000000...Done. 2764.4571163654327 total seconds elapsed\n",
      "importing rows 14000000 to 15000000...Done. 2959.6932983398438 total seconds elapsed\n",
      "importing rows 15000000 to 16000000...Done. 3156.239888906479 total seconds elapsed\n",
      "importing rows 16000000 to 17000000...Done. 3355.2837133407593 total seconds elapsed\n",
      "importing rows 17000000 to 18000000...Done. 3561.9112389087677 total seconds elapsed\n",
      "importing rows 18000000 to 19000000...Done. 3757.25905919075 total seconds elapsed\n",
      "importing rows 19000000 to 20000000...Done. 3954.126307964325 total seconds elapsed\n",
      "importing rows 20000000 to 21000000...Done. 4149.138860464096 total seconds elapsed\n",
      "importing rows 21000000 to 22000000...Done. 4343.818372964859 total seconds elapsed\n",
      "importing rows 22000000 to 23000000...Done. 4538.666905641556 total seconds elapsed\n",
      "importing rows 23000000 to 24000000...Done. 4732.879355192184 total seconds elapsed\n",
      "importing rows 24000000 to 25000000...Done. 4929.182502985001 total seconds elapsed\n",
      "importing rows 25000000 to 26000000...Done. 5123.8926911354065 total seconds elapsed\n",
      "importing rows 26000000 to 26024289...Done. 5128.694673061371 total seconds elapsed\n"
     ]
    }
   ],
   "source": [
